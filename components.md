...
## How to use `components.yaml`

The companion `components.yaml` file contains the same data in a structured, machine-readable format (`component_key`, `node_id`, `variant_axes`, etc.) — useful for scripting, design-token tooling, or feeding into `figma_instantiate_component` / `figma_get_component_details`.
